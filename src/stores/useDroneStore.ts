...
import { getDrones, createDrone, updateDrone, deleteDrone, getDroneById, getStatus } from '@/api/models/drone/droneEndpoint';
...
export const useDroneStore = create<DroneState>((set, get) => ({
  ...
  fetchDrones: async (params?: Record<string, any>) => {
    ...
    try {
      ...
      if (!totalHeader) {
        console.warn('X-Total header not found in response, using drones.length as fallback');
      }
