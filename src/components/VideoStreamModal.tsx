...
export default function VideoStreamModal({
    ...
    useEffect(() => {
        ...
        const handleLoadedMetadata = () => {
            console.log(
                `[VideoStream] Video metadata loaded: ${video.videoWidth}x${video.videoHeight}`,
            );
        };
        ...
        const handleError = () => {
            console.error(
                '[VideoStream] Video element error:',
        ...
        if (jmuxerRef.current && typeof jmuxerRef.current.on === 'function') {
            jmuxerRef.current.on('ready', () => {
                ...
                setTimeout(() => {
                    if (video.src) {
                        console.log(
                            `[VideoStream] Video src set: ${video.src.substring(0, 50)}...`,
                        );
                    } else {
    ...
    const handleVideoFrame = useCallback((data: string) => {
        try {
            console.log(
                '[VideoStream] Frame received, data type:',
                typeof data,
                'length:',
                data?.length,
            );
            ...
            if (!jmuxerRef.current) {
                console.warn(
                    '[VideoStream] JMuxer instance is null, dropping frame. This should not happen if subscribe is called correctly.',
                );
                return;
            }
            ...
            if (!isValidH264 && frameCountRef.current < 3) {
                console.warn(
                    '[VideoStream] Data does not look like H.264 NAL unit. First bytes:',
                    Array.from(firstBytes)
                        .map(b => '0x' + b.toString(16).padStart(2, '0'))
                        .join(' '),
                );
            ...
            if (jmuxerRef.current) {
                try {
                    ...
                    if (frameCountRef.current === 0) {
                        console.log(
                            `[VideoStream] First frame fed to jmuxer (${bytes.length} bytes)`,
                        );
                        ...
                        setTimeout(() => {
                            ...
                            if (video) {
                                console.log(
                                    `[VideoStream] Video state: readyState=${video.readyState}, paused=${video.paused}, currentTime=${video.currentTime}`,
                                );
                    ...
                    if (frameCountRef.current % 30 === 0) {
                        ...
                        if (video) {
                            console.log(
                                `[VideoStream] Frame #${frameCountRef.current}: readyState=${video.readyState}, paused=${video.paused}`,
                            );
                        }
                    }
                } catch (e) {
    ...
    useEffect(() => {
        ...
        console.log(
            '[VideoStream] Subscribing to video:frame event for drone:',
            droneId,
            'jmuxer initialized:',
            jmuxerInitialized,
        );
    ...
    const handleLocationUpdate = useCallback(
        (data: { droneId: string; location: any }) => {
                        ...
                        <Alert variant="destructive">
                            ...
                            <AlertDescription>
                                ...
                                <ul className="list-disc list-inside mt-2 space-y-1">
                                    ...
                                    <li>
                                        URL kết nối có đúng không? (kiểm tra VITE_WS_URL trong .env)
                                    </li>
