...
export function useLocalFakeTelem(
  ...
  useEffect(() => {
    ...
    function metersToDegLat(m:number){return m/111320;}
    function metersToDegLon(m:number, lat:number){return m/(111320*Math.cos(lat*Math.PI/180));}
    function randomPointNear(c:any, r=1500){
      const a = Math.random() * Math.PI*2; const rr = r*Math.sqrt(Math.random());
      const dx = Math.cos(a)*rr; const dy = Math.sin(a)*rr;
    ...
    intervalRef.current = window.setInterval(()=>{
      ...
      for (const d of dronesRef.current){
        ...
        d.lat += metersToDegLat(dy); d.lon += metersToDegLon(dx, d.lat);
        d.altitude = Math.max(5, d.altitude + rand(-1,1));
        d.battery = Math.max(0, d.battery - dt*rand(0.01,0.07));
        d.timestamp = Date.now();
        out.push({
          id: d.id, lat: d.lat, lon: d.lon, heading: Math.round(d.heading),
          speed: Math.round(d.speed*10)/10, battery: Math.round(d.battery),
          altitude: Math.round(d.altitude*10)/10, timestamp: d.timestamp
        });
      }
